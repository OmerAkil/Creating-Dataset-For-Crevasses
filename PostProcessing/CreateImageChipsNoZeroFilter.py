...
def create_image_chips(
    ...
    for image_id in images:
        ...
        image_data = np.zeros((n_tiles, kernel, kernel), dtype=np.uint16)
        label_data = np.zeros((n_tiles, kernel, kernel), dtype=np.uint16)
        fracture_density = np.zeros((n_tiles,), dtype=np.float32)
        ...
        for i in range(n_tiles):
            ...
            if i == 0:
                print(f"  First chip indices: ys0={ys0}, ye0={ye0}, xs0={xs0}, xe0={xe0}")
                print(f"  First chip min/max = {chip.min()} / {chip.max()}")

            # WorldView chip
            image_data[i, :, :] = chip

            # Binary fracture map chip
            seg = frac_img[ys0:ye0, xs0:xe0]
            label_data[i, :, :] = seg

            # Fracture density = mean of fracture map in chip
            # If frac_img is 0/1, this is fraction of pixels with fractures.
            fracture_density[i] = float(seg.mean())
